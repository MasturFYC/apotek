...
import {
  OrderContextType,
  OrderProvider,
} from "../../components/context/order-context";
...
const OrderPage: React.FunctionComponent<OrderPageParam> = ({
  salesmans,
  customers,
  methods,
}) => {
  ...
  const refreshData = async (
    data: iOrderDetail,
    method: string,
    callback?: (data: iOrderDetail | null) => void
  ) => {
    ...
    if (res.status !== 200) {
      ...
      if (order && order.details) {
        switch (method) {
          ...
          case "DELETE":
            {
              const delDetail = order.details.filter(
                (x) => x.id === data.id
              )[0];
        ...
        <div className={"container mt-5"}>
          <div className={"row ms-2"}>
            <TabStyle
              isSelected={tab === 0}
              onClick={() => setTab(0)}
              className={"col-auto rounded-top"}
            >
              Details
            </TabStyle>
            <TabStyle
...
const UseOrder = (id: number) => {
  ...
  const { data, error, mutate } = useSWR<iOrder, Error>(
    baseUrl,
    fetcher,
    revalidationOptions
  );
